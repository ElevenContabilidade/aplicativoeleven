...
export function MeiChecklist() {
  ...
  const checklist = useAppStore((s) => s.checklistMei);
  ...
  function pctFor(clienteId: string, comp: string) {
  ...
  const clientPcts = useMemo(
    () => myClients.map((c) => ({ cliente: (c.dados.nomeFantasia ?? c.dados.razaoSocial).split(" ").slice(0, 2).join(" "), pct: pctFor(c.id, competencia) })),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [myClients, checklist, competencia]
  );
  ...
  return (
    <>
      <Card className="mt-4">
        <CardHeader><CardTitle>Dashboard MEI — {MESES.find((m) => m.value === mes)?.label}/{year}</CardTitle></CardHeader>
        <CardContent className="space-y-4 pt-4">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <div>
              <p className="mb-2 text-xs font-semibold text-sand-700">Evolução mensal (média da carteira MEI) — {year}</p>
              <div className="h-56 w-full">
                <ResponsiveContainer>
      ...
      <Card className="mt-4">
        <CardHeader>
          <CardTitle>Checklist mensal dos clientes MEI</CardTitle>
              ...
              <tbody>
                {myClients.map((c) => (
                  <ClientRow
                    ...
                    statusFor={statusFor}
                    setChecklistMei={setChecklistMei}
                    pct={pctFor(c.id, competencia)}
                  />
                ))}
                {myClients.length === 0 && (
                  <tr>
                    <td colSpan={rotinas.length + 2} className="py-8 text-center text-sand-400">
                      Nenhum cliente MEI ativo na carteira.
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {rotinas.map((r) => {
        ...
        return (
          <td key={r} className="border-b border-l border-sand-200 px-2 py-1.5">
            <Select
              value={status ?? "—"}
              onValueChange={(v) => setChecklistMei(c.id, competencia, r, v === "—" ? null : (v as ChecklistStatus))}
            >
